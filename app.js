...
function detectAnchorCenter(gray, box){
  ...
  const nz = cv.countNonZero(bw);
  let cx = (box.x + box.w / 2) * gray.cols;
  let cy = (box.y + box.h / 2) * gray.rows;
  if (nz > 8 && m.m00 > 0) {
    cx = box.x * gray.cols + (m.m10 / m.m00);
    cy = box.y * gray.rows + (m.m01 / m.m00);
  }
  r.delete(); bw.delete();
  return { cx, cy, strength: nz };
}

function rectifyByAnchors(gray, boxes) {
  const keys = ['top1', 'top3', 'bot1', 'bot2'];
  const srcPts = [];
  const dstPts = [];
  for (const k of keys) {
    const c = detectAnchorCenter(gray, boxes[k]);
    srcPts.push(c.cx, c.cy);
    dstPts.push((boxes[k].x + boxes[k].w / 2) * gray.cols, (boxes[k].y + boxes[k].h / 2) * gray.rows);
  }
  const src = cv.matFromArray(4, 1, cv.CV_32FC2, srcPts);
  const dst = cv.matFromArray(4, 1, cv.CV_32FC2, dstPts);
  const M = cv.getPerspectiveTransform(src, dst);
  const warped = new cv.Mat();
  cv.warpPerspective(gray, warped, M, new cv.Size(gray.cols, gray.rows), cv.INTER_LINEAR, cv.BORDER_REPLICATE);
...
function detectOrientationRobust(gray, boxes){
  ...
  let best = { score: -Infinity, mat: current.clone(), tag: 'fallback' };
  for(let tries=0; tries<4; tries++){
    ...
    if (topOk && !botOk) {
      ...
      return { gray: current.clone(), orientation: `pattern-locked@${tries*90}` };
    }
  ...
  current.delete();
  return {gray: best.mat, orientation:`fallback-${best.tag}`};
}
...
function decodeAnswers(gray, boxes){
  ...
  state.answerAreas.filter(a=>a.active).sort((a,b)=>a.start-b.start).forEach((a)=>{
    ...
    for(let qi=0; qi<qc; qi++){
      ...
      const selected = inks
        .map((ink, i) => ({ ink, i, fillPct: (ink / 255) * 100 }))
        .filter(({ink, fillPct}) => ink >= markTh && ink >= (maxInk * 0.8) && fillPct >= 75)
        .map(v => v.i);
...
function initSupabaseClient(){
  ...
  if (key.startsWith('sbp_')) {
    log('Token sbp_ adalah Personal Access Token (untuk management API), bukan untuk query tabel. Gunakan Publishable/Anon key dari Project Settings > API.');
    return null;
  }
